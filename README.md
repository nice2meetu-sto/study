# 공부의 별 ⭐

1인용 공부 기록 웹앱. 공부 타이머 + 위클리 플랜 + 통계 + 과목별 할일/인강 관리.

- 프론트엔드: HTML + CSS + vanilla JS (프레임워크 없음, ES 모듈)
- 백엔드: Supabase (Postgres + REST + Auth, 모든 테이블 RLS 적용)
- 호스팅: GitHub Pages (정적)

## 처음 한 번만 하면 되는 설정

### 1. Supabase 테이블 만들기
Supabase 대시보드 → **SQL Editor** → [`supabase/schema.sql`](supabase/schema.sql) 내용 전체를 붙여넣고 **Run**.
(여러 번 실행해도 안전합니다)

### 2. 로그인 계정 만들기
Supabase 대시보드 → **Authentication → Users → Add user**
- 이메일 + 비밀번호 입력, **Auto Confirm User** 체크 후 생성
- 이 계정으로 앱에 로그인합니다
- (선택) 혼자만 쓰려면 **Authentication → Sign In / Up** 에서 *Allow new users to sign up* 을 꺼두세요
- (선택) 홈 인사말에 이름을 띄우려면 해당 유저의 **User Metadata**에 `{"name": "수민"}` 을 추가하세요

### (선택) 앱처럼 설치하기 (PWA)
사이트 접속 후:
- **갤럭시 (Chrome/삼성인터넷)**: 메뉴(⋮ 또는 ≡) → **"홈 화면에 추가"** 또는 **"앱 설치"**
- **아이폰 (Safari)**: 공유 버튼 → **"홈 화면에 추가"**

홈 화면의 별이 아이콘으로 열면 주소창 없이 전체 화면 앱처럼 실행됩니다.

### 3. GitHub Pages 켜기
저장소 **Settings → Pages → Deploy from a branch** → `main` / root 선택.
잠시 후 `https://<계정>.github.io/study/` 로 접속할 수 있습니다.

## 파일 구성

```
index.html          앱 셸 (로그인 + 5개 탭 화면 + 시트)
css/style.css       디자인 (목업 v8 토큰 그대로)
js/config.js        Supabase URL / anon key
js/api.js           데이터 레이어 (supabase-js 클라이언트 + 전체 조회 헬퍼)
js/app.js           앱 로직 (홈 / 플랜 / 타이머 / 기록 / 과목 / 설정 시트)
supabase/schema.sql 테이블 + 인덱스 + RLS 정책
```

## 동작 규칙 (스펙 해석 메모)

- **타이머**는 시작 시각 기준(`Date.now() - startAt + base`)으로 계산하고 localStorage에 상태를 저장하므로, 화면이 꺼지거나 새로고침해도 이어서 갑니다. 종료 버튼 → 확인 시트에서 **기록하기**(저장) 또는 **취소**(이번 기록 삭제)를 선택합니다.
- **홈 자유 메모**는 Supabase 유저 메타데이터에 저장됩니다 (별도 테이블 불필요).
- **저장 자동 재시도**: 네트워크 오류로 저장에 실패하면 0.8초/1.6초/3.2초 간격으로 최대 3회 재시도하고, 그래도 실패하면 알림을 띄웁니다.
- **배정 완료 ↔ 할일 완료 동기화**: 플랜(카드 탭)/홈에서 완료하면 할일 원본(todos)도 완료 처리됩니다. 과목 페이지에서 할일을 체크하면 오늘 날짜의 배정도 함께 완료됩니다.
- **플랜 배정 규칙**: 할일 하나는 한 주에 한 요일만(주별로 각각 가능). 배정 카드는 탭=완료 토글, 드래그=요일 이동·순서 변경·담기로 해제. 지난 주는 읽기 전용입니다.
- **완료 연동**: 할일이 완료되면 미래 날짜의 미완료 배정은 자동 삭제됩니다(오늘·과거 기록 유지). 과목 탭에서 배정 없는 할일을 완료하면 '오늘 한 일'로 자동 기록되고, 바로 체크를 취소하면 그 기록도 회수됩니다.
- **기록 보존**: 배정은 할일 이름 스냅샷을 보관하므로, 할일을 삭제해도 플랜·기록의 과거 기록은 남습니다.
- **과목을 삭제해도** 그 과목의 공부 세션 기록은 남고, 통계에서 "삭제된 과목"으로 표시됩니다. (할일·인강·배정은 과목과 함께 삭제)
- **완주한 인강**은 과목 카드에서는 숨겨지고, 과목 탭의 "인강" 필터에서 "수강 완료!"로 표시됩니다.
- **공부량 6단계**: 0분(회색) / ~2시간 / ~4시간 / ~6시간 / ~8시간 / 8시간 초과(주황) — 홈 히트맵과 기록 달력이 같은 스케일을 씁니다. 기록 달력의 별이 표정은 ~4시간 입만 웃음, ~8시간 눈웃음, 8시간 초과 >_< 입니다.
- 날짜 경계는 기기 로컬 자정 기준입니다.
